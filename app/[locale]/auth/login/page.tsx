...
import { signInWithEmail, signInWithGoogle, signInWithPassword, signUpWithPassword } from '@/app/actions/auth'
...
function EmailPasswordForm({ origin }: { origin: string }) {
  ...
  const actionToUse = useMemo(() => {
    switch (authMethod) {
      case 'login': return signInWithPassword
      case 'signup': return signUpWithPassword
      case 'magic': return signInWithEmail
    }
  }, [authMethod])
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center p-5">
      <div className="w-full max-w-sm">
        ...
        {/* Google Login */}
        <form action={googleAction} className="mb-6">
          <input type="hidden" name="origin" value={origin} />
          <Button
            type="submit"
            variant="outline"
        ...
        <EmailPasswordForm origin={origin} />
